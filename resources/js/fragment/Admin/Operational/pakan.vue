    ...
    <div class="row-span-10 bg-gray-100  h-[95vh]   overflow-x-auto overflow-y-auto p-0 m-0">
        <div class="divide-y divide-x divide-gray-300 border-collapse">
                <table>
                    ...
                    <tbody>
                        <tr v-for="(item, index) in pakanlist" :key="item.id"  @click="selectItem(item.id)"
...
const pakanlist = computed(() =>
  props.pakan.map(item => ({
    ...
    const downloadItem = () => {
        ...
        axios.get(`/download/pakan`, { responseType: 'blob' })
        .then((response) => {
          ...
          link.href = url;
          link.setAttribute('download', `pakan.xlsx`); // Nama file

    ...
    <div class="row-span-10 bg-gray-100  h-[95vh] overflow-x-auto overflow-y-auto p-0 m-0">
        <div class="divide-y divide-x divide-gray-300 border-collapse">
            <table>
                ...
                <tbody>
                    <tr v-for="item in kandanglist" :key="item.id">
                        <td :class="classestd">
                            {{ item . id }}
                        </td>
                        <td :class="classestd">
...
<script setup>
    ...
    const downloadItem = () => {
        ...
        axios.get(`/download/kandang`, { responseType: 'blob' })
        .then((response) => {
          ...
          link.href = url;
          link.setAttribute('download', `kandang.xlsx`); // Nama file

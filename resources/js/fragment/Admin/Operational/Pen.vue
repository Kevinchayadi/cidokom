    ...
    <div class="overflow-x-auto overflow-y-auto h-screen">
        ...
        <div class="divide-y divide-x divide-gray-300 border-collapse">
                <table>
                    ...
                    <tbody>
                        <tr v-for="item in penlist" :key="item.id">
                            <td :class="classestd">
                                {{ item.id }}
                            </td>
                            <td :class="classestd">
...
<script setup>
    ...
    const classesth =
        ' bg-blue-300 text-center  text-xs border-gray-300 text-table font-medium text-gray-700 uppercase tracking-wider  sticky top-[24px] min-w-[120px] shadow-[inset_1px_-1px_1px_white]'
    const classesth2 =
...
const penlist = computed(() =>
  props.pen.map(item => ({
    ...
    const downloadItem = () => {
        ...
        axios.get(`/download/pen`, { responseType: 'blob' })
        .then((response) => {
          ...
          link.href = url;
          link.setAttribute('download', `pen.xlsx`); // Nama file

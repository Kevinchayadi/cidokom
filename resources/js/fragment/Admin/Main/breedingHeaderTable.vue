...
<script setup>
    ...
    // const data = computed(()=>
    //     const update =new Date(item.breeding_details[0].updated_at).toISOString().split('T')[0] ?? new Date(item.updated_at).toISOString().split('T')[0],
    ...
    props.breeding.map(item => {
        ...
        // Tentukan lastupdate
        const lastUpdate = formatDate(breedingDetailsUpdatedAt) || formatDate(itemUpdatedAt);

        ...
        <table class=" divide-y divide-x divide-gray-300 border-collapse">
            ...
            <tbody class="bg-gray-100 divide-y divide-gray-200 custom-scroll">
                <tr v-for="(item, index) in data" :key="index"
                    ...
                    <!-- Data Columns -->
                    <td class="md:sticky md:left-[75px]"
                        :class="{ 'bg-gray-300': selectedId === item.id, 'bg-white': selectedId !== item.id, [classestd]: true }">
                        {{ item . date }}
                    </td>
                    ...
                    <td class="md:sticky md:left-[225px]"
                        :class="{ 'bg-gray-300': selectedId === item.id, 'bg-white': selectedId !== item.id, [classestd]: true }">
                        {{ selectedData . jumlah_jantan }}
                    </td>
                    <td class="md:sticky md:left-[300px]"
                        :class="{ 'bg-gray-300': selectedId === item.id, 'bg-white': selectedId !== item.id, [classestd]: true }">
                        {{ item . last_female }}
                    </td>
                    <td class="md:sticky md:left-[375px]"
                        :class="{ 'bg-gray-300': selectedId === item.id, 'bg-white': selectedId !== item.id, [classestd]: true }">
                        {{ item . last_male }}
                    </td>
                    <td
    ...
    const data = computed(() =>
        detailData.value.map(item => ({
            ...
            egg_percentage: ((item.egg_afternoon + item.egg_morning) / item.last_female * 100).toFixed(2) +
                '%',
            cracked: item.broken,
            cracked_percentage: parseFloat(((item.broken / (item.egg_afternoon + item.egg_morning)) * 100)
                .toFixed(2)) + '%',
            abnormal: item.abnormal,
            abnormal_percentage: ((item.abnormal / (item.egg_afternoon + item.egg_morning)) * 100).toFixed(
                2) + '%',
            sale: item.sale,

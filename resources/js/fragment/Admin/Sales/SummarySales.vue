<template>
    ...
    <div class="row-span-10 bg-gray-100 h-[95vh] overflow-x-auto overflow-y-auto px-5 m-0">
        ...
        <div class="tables-container flex flex-col md:flex-row justify-between gap-4 mt-5 mb-2">
            <div>
                ...
                <div class="table-wrapper flex-1 overflow-auto min-h-[35vh] max-h-[35vh] justify-items-center">
                    <table>
                        <thead>
                            <tr>
                                <th :class="classesth"  class="cursor-default">No.</th>
                                <th :class="classesth" class="cursor-default" @click="sortBy('nama')">
                                    Nama Pelanggan
                                    <span v-if="sortColumn === 'nama'" class="select-none cursor-default">
                                        {{ sortDirection === 'asc' ? '↑' : sortDirection === 'desc' ? '↓' : '' }}
                                    </span>
                                </th>
                                <th :class="classesth" class="cursor-default" @click="sortBy('qty')">
                                    Qty
                                    <span v-if="sortColumn === 'qty'" class="select-none cursor-default">
                                        {{ sortDirection === 'asc' ? '↑' : sortDirection === 'desc' ? '↓' : '' }}
                                    </span>
                                </th>
                                <th :class="classesth" class="cursor-default" @click="sortBy('price')">
                                    Price
                                    <span v-if="sortColumn === 'price'" class="select-none cursor-default">
                                        {{ sortDirection === 'asc' ? '↑' : sortDirection === 'desc' ? '↓' : '' }}
                                    </span>
                                </th>
                            </tr>
...
<script setup>
    ...
    const sortColumn = ref(null); // misal: 'qty'
    ...
    const recentCustData = computed(() => {
        let data = (props.recentCust ?? []).map((item) => ({

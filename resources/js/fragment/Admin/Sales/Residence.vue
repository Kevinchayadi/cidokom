<template>
    <div class="flex justify-between sticky px-2 py-1 top-0 left-0 bg-primary">
        ...
        <div class="flex gap-1 justify-center">
            <Search v-model="search" place="Residence Name"/>
        </div>
    </div>
    <div
...
const originalresidencelist = computed(() =>
...
const residencelist = ref([...originalresidencelist.value]);
...
watch(search, (newVal) => {
    ...
    if (!keyword) {
        residencelist.value = [...originalresidencelist.value];
    } else {
        residencelist.value = originalresidencelist.value.filter((item) =>

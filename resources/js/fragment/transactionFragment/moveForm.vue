<template>
    <div class="w-[80%]">
        ...
        <form @submit.prevent="handleSubmit">
            <div v-if="Object.keys(props.chicken).length === 2" class="flex w-full justify-around my-5 text-center">
                <h2 class="lg:text-xl text-lg text-yellow-300 font-bold">
                    Male Chicken : {{ chicken.male }} Pcs
                </h2>
                <h2 class="lg:text-xl text-lg text-yellow-300 font-bold">
                    Female Chicken : {{ chicken.female }} Pcs
                </h2>
            </div>
            <div v-else class="flex w-full justify-center my-5 text-center">
                <h2 class="lg:text-xl text-lg text-yellow-300 font-bold">
                    Male/Female Chicken : {{ chicken.total }} Pcs
                </h2>
            ...
            />
            <InputFragment
                v-model="formData.total_male_move"
                name="male"
                content="male chicken"
                type="number"
            />
            <InputFragment
                v-model="formData.total_female_move"
                name="female"
